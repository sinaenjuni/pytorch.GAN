...
class Discriminator(nn.Module):
    def __init__(self, img_size, d_conv_dim, apply_d_sn, apply_attn, attn_d_loc, d_cond_mtd, aux_cls_type, d_embed_dim, normalize_d_embed,
        ...
        if self.MODEL.info_type in ["discrete", "both"]:
            ...
            self.info_discrete_linear = ops.linear(in_features=self.out_dims[-1], out_features=out_features, bias=False)
        if self.MODEL.info_type in ["continuous", "both"]:
            out_features = self.MODEL.info_num_conti_c
            self.info_conti_mu_linear = ops.linear(in_features=self.out_dims[-1], out_features=out_features, bias=False)
            self.info_conti_var_linear = ops.linear(in_features=self.out_dims[-1], out_features=out_features, bias=False)
