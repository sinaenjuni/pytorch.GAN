...
class Generator(nn.Module):
    ...
    def __init__(self, image_size, image_channel, std_channel, latent_dim, sn, bn):
        ...
        if sn:
            self.layer5 = nn.Sequential(spectral_norm(nn.ConvTranspose2d(std_channel*1, image_channel, kernel_size=4, stride=2, padding=1), eps=1e-6))   # 32*32*3
        else:
