...
train_data_loader = ImbalanceCIFAR10DataLoader(data_dir='../../data', batch_size=128,
                                        shuffle=True, num_workers=4, training=True,
                                        imb_factor=0.01)
test_data_loader = ImbalanceCIFAR10DataLoader(data_dir='../../data', batch_size=128,
...
test_maximum_acc = 0
test_maximum_acc_epoch = 0

# Start training
total_step = len(train_data_loader)
for epoch in range(num_epochs):
    train_maximum_acc = 0
    train_maximum_acc_epoch = 0
    ...
    for i, (images, labels) in enumerate(train_data_loader):
        ...
        #Accuracy
        one_hot = out_cls.argmax(-1)
        acc = (one_hot == labels).float().mean()
    ...
    test_acc = 0
    test_total = 0
    for i, (test_images, test_labales) in enumerate(test_data_loader):
        test_images = test_images.to(device)
        test_labales = test_labales.to(device)


        out_adv, out_cls = D(test_images)

        out_cls = out_cls.squeeze()
        ont_hot = out_cls.argmax(-1)
        test_acc += (ont_hot == test_labales).sum().item()
        test_total += test_labales.size(0)
    test_acc = test_acc / test_total
    if maximum_acc < test_acc:
        maximum_acc = test_acc
        maximum_acc_epoch = epoch
    print(f"Test accuracy: {test_acc}, Miximum epoch/accuracy: {maximum_acc_epoch}/{maximum_acc}")
