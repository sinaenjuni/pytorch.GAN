...
print(fixed_noise.size(), fixed_onehot.size())
...
class Discriminator(nn.Module):
    def __init__(self, image_size, nc, num_class):
        ...
        self.input_layer = nn.Sequential(
            nn.Linear(in_features=input_size + num_class, out_features=256),
            nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.layer1 = layer(in_channel=256, out_channel=128,
                            activation=nn.LeakyReLU(negative_slope=0.2, inplace=True))
        self.output_layer = nn.Sequential(
            nn.Linear(in_features=128, out_features=nc), nn.Sigmoid())
...
for epoch in range(epochs):
    for i, (images, target) in enumerate(data_loader):
        ...
        d_loss_real = bce_loss(d_output_real, real_labels)
